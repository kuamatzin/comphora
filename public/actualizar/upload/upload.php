...
function get_planes()
{
    ...
    include 'con.php';
    	...
    	for($i=1;$i<count($names);$i++){
    		...
    		$query="Select id From planes Where get_nombre_plan(t_plan)='".$names[$i]."' and status=1;";
    		//echo "query:".$query."<br>";
...
function Decide($plan, $smartphone, $meses, $valor)
{
	...
	// tal vez siempre se tenga que hacer insert, en todo caso el plan debe de tener un status 1 para que siga vigente y entonces asignarle un smartphone en costo_agregado
		...
		if(strcmp($phone, "")!=0){
			...
			for ($j=1; $j < count($info); $j++) {
				...
				if(strcmp($GLOBALS["planes_ids"][$j-1], "")!=0){
					...
					$info[$j]=str_replace("NA","NULL",$info[$j]);//NULL
					$info[$j]=str_replace("INCLUIDO","0",$info[$j]);//GRATIS
				...
				else
				{
					...
					//array_push($aux_array[$GLOBALS["planes"][$j]],$info[$j]);
					$cont++;
				}
			}
...
//	 echo "info de smartphones que faltan:<br>";print_r($smartphones_faltan_info);echo "<br>";
	 ...
	 //$GLOBALS["smartphones_f"]=$return["smartphones_faltan_info"];
	 escribir_archivo($return["smartphones_faltan_info"],"smartphones_faltan.csv");
	 escribir_archivo($return["planes_faltan_info"],"planes_faltan.csv");
...
function escribir_archivo($array,$name)
{

	//$array= $GLOBALS["smartphones_f"];
	//print_r($array);
	$myfile = fopen($name, "w");
